...
function branch_status() {
	local a=$1 b=$2
	echo "branch_status of ${a} of ${b}" >&2
...
do
	...
	echo "branch_status result is $retval"
...
if [ $num -gt 0 ]; then
	echo `cat $logfileName.err | grep -v From | grep -v "\->" | grep -v "git help gc"`
...
if [ ! -d "$package_dir" ]; then
	# mkdir $package_dir
	ln -s $PWD/../project/client/$package_dir/ $PWD/$package_dir
fi
